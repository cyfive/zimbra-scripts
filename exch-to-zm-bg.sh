...
for user_line in $users_list; do
	...
	if [ $? -eq 0 ]; then
		...
		workers=`ps a | grep imapsync | grep -v grep | wc -l`
		next=0
		while [ $next -eq 0 ]; do
			if [ $workers -le $MAXWORKERS ]; then
				echo "Запускаем фоновый процесс..."
				$IMAPSYNC --host1 $exch_server --user1=$user_name --authmech1=PLAIN --password1=$user_pass --host2=$zm_server --authmech2=PLAIN --ssl2 --user2=$user_name --password2=$user_pass --nocheckmessageexists --nofoldersizes --nofoldersizesatend > /dev/null 2>&1 &
				next=1
			else
				echo "Ждем свободный процесс..."
				sleep 1m
				workers=`ps a | grep imapsync | grep -v grep | wc -l`
			fi
		done
